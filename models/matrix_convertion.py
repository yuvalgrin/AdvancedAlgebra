...
def inverse_matrix(mat,p):
    ...
    d_inv = prime_field_element.PrimeFieldElement(1,p)/prime_field_element.PrimeFieldElement(det(mat.astype(int)*1.01).astype(int),p)
    #print(d_inv)
    #print(np.matmul(inverse,mat))
    #print("mat:",mat.astype(int))
    #print("inv:",inverse.astype(int))
    inverse = [[prime_field_element.PrimeFieldElement(x, p) * d_inv for x in y] for y in inverse]
    return inverse
